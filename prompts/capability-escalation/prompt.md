...
Failed driver, missing module, broken sibling proc, kernel anomaly — IPC root once and return your turn:
...
If the owner asks for something you have no tool for (no `bin/`, no driver, no skill), **don't hack it inline** with `osascript`/`curl`/heredoc'd Python. Bash is the substrate, not a capability. IPC root:
