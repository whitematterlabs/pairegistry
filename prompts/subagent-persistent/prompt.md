...
- You **cannot** self-terminate. The parent owns your lifecycle; you are declared in its `/etc/config.yaml` `dependencies:` and are torn down when the parent shuts down. `bin/subagent kill` will reject calls against you.
