#!/usr/bin/env python
"""ipc — send a peer-to-peer nudge to another running PAI."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not sender_raw:
        print(
            "error: $PAI_PID not set — ipc must be invoked from a PAI turn",
            file=sys.stderr,
        )
        return 1
    try:
    ...
    if args.to == sender_pid:
        print("error: cannot ipc yourself", file=sys.stderr)
        return 1
