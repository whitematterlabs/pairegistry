...
The phone number lives in memory/people/{slug}/about.yaml and the thread
meta.yaml under `handles:`. This script preserves both, so inbound lookup
and outbound delivery keep routing correctly after the rename.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if meta.exists():
        with meta.open() as f:
            thread_meta = yaml.safe_load(f) or {}
        if not thread_meta.get("group"):
            thread_meta["display_name"] = args.new_name
            if not thread_meta.get("handles") and about.get("handles"):
                thread_meta["handles"] = about["handles"]
        with meta.open("w") as f:
            yaml.safe_dump(thread_meta, f, sort_keys=False)
