...
mail. The `mailv2` driver keeps a **complete on-disk archive** of every
...
`~/drafts/`, and the `mailv2-out` driver saves it into Mail.app's Drafts
...
**`body_state`** is the one field unique to mailv2. `present` means the
...
whole month. There is **no `mailsearch` for mailv2** — the archive is
